...
pub trait PrinterDevice: Send + Sync {
    ...
    /// Keep the printer awake with a lightweight status request (also samples battery/film).
    ///
    /// NB: this does not stop the Instax auto-power-off after ~4.5 min idle. An experiment adding
    /// a benign LED-off write confirmed no write resets the printer's idle timer either — there is
    /// no documented wake-lock command (see bridge/docs/printer-protocol-notes.md). Sleep is
    /// handled by fast reconnect + auto-rebond on demand, not by holding the link open.
